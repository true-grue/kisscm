...
Сначала воспользуемся утилитой командной строки Linux `cat` для получения в stdout строк из файла `/var/log/syslog`, содержащего сообщения о происходящих в системе событиях @unixbook. Также подсчитаем число строк в журнале событий:
...
При помощи стандартной утилиты `time` @unixbook легко выполнить замеры времени выполнения реализованной утилиты по результатам 100 тестовых запусков поиска подстроки `root` в текстовом файле `/var/log/syslog`:
